...
substrate_df = pd.read_csv('./substrate.csv')
...
# org.to_csv('organisms.csv', index=False)

def exact_match(organism):
# Exact matching
    substrate_subsites_filtered = substrate_subsites[substrate_subsites['organism'] == organism]
    matches = substrate_subsites_filtered[substrate_subsites_filtered['Cleavage Site'].isin(nontryp_termini['Non-Tryptic Termini'])]
    print(matches)

# exact_match('Klebsiella pneumoniae')
...
def fuzzy_match(organism):
    substrate_subsites_filtered = substrate_subsites[substrate_subsites['organism'] == organism].reset_index(drop=True)
    input_termini = nontryp_termini['Non-Tryptic Termini']
    ...
    for termini in input_termini:
        for _, (i, substrate) in tree.find((None, termini), 1):  # Find matches -> set Levenshtein distance to 2
            matches.append((termini, substrate, substrate_subsites_filtered.loc[i,'Protease']))

    matches_df = pd.DataFrame(matches, columns=['Non-Tryptic Termini', 'Cleavage Site', 'Protease'])
    matches_df.to_csv('fuzzy_matches.csv', index=False)
    return matches_df
fuzzy_match("Homo sapiens")

# Fuzzy-matching with BLOSUM substitution matrix scoring